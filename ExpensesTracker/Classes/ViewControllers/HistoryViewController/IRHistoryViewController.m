...
- (void)configureHeaderCell:(IRHistoryHeaderTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch ([IRCommon getGroupingName]) {
        case WeeklyGrouping:{
            //NSString *formatTemplate = [NSDateFormatter dateFormatFromTemplate:@"dd-MM-yyyy" options:0 locale:[NSLocale currentLocale]];
          ...
          //  IRCommon getDate:expense.date
            ...
            //NSString *prova;
        //    IRExpense *irExpense = irExpense.date;
            //prova = [formatter stringFromDate:expense.date];
            
          //  prova = cell.headerText.text;
            //NSLog(@"prova:   %@",prova);
            titleString = @"Mostra";
        }
            break;
    ...
    cell.profit.text = @" ";
    //[NSString stringWithFormat:@"%@ %0.2f",[IRCommon getCurrencySymbolFromCode:[[NSUserDefaults standardUserDefaults]objectForKey:@"currency"]],fabsf(profit)];
...
- (void)configureCell:(IRHIstoryTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath
{
    ...
    // expenseModal.currency show for each rows
    cell.amount.text = [NSString stringWithFormat:@"%@ %0.2f",expenseModal.currency,expenseModal.amount];
    ...
    if ([IRCommon isToday:expense.date]) {
        ...
        dateString = [NSString stringWithFormat:@"Tomorrow"];
    } else {
