...
{
    NSString *cString = [[NSUserDefaults standardUserDefaults]objectForKey:@"currency"];
    const char *myCharDefault = [cString cStringUsingEncoding:NSASCIIStringEncoding];

    const char *myCharEUR = "EUR";
    const char *myCharUSD = "USD";
    const char *myCharGBP = "GBP";
    
    double totalExpense = 0;
    for (IRExpense *expense in expenseArray) {
        //debug1
      //  NSLog(@"---------- DEFAULT: %@ ---------------",myCharDefault);
       

        if (strcmp(myCharDefault, expense.currency.UTF8String)){
            NSLog(@"---------------- ciao USD: %@-----------------",expense.currency);
            
            if(strcmp(myCharEUR, expense.currency.UTF8String)){
                totalExpense = (expense.isExpense) ? (totalExpense - (expense.amount*0.8633)) : (totalExpense + (expense.amount*0.8633));
                
            }
            else if(strcmp(myCharUSD, expense.currency.UTF8String)){
                totalExpense = (expense.isExpense) ? (totalExpense - (expense.amount*1.15461)) : (totalExpense + (expense.amount*1.15461));
                
            }
            
            else if(strcmp(myCharGBP, expense.currency.UTF8String)){
                totalExpense = (expense.isExpense) ? (totalExpense - (expense.amount*0.8791)) : (totalExpense + (expense.amount*0.8791));
